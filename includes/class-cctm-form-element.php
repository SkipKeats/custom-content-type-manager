...
abstract class CCTM_Form_Element {
	...
	/**
	 * CSS Element Increment
	 *
	 * Used to increment CSS ids as we wrap multiple elements.
	 *
	 * @var int $element_i Number.
	 */
	public $element_i = 0;

	/**
	 * Desription of Common Field Elements
	 *
	 * Contains reusable localized descriptions of common field definition elements, e.g. 'label'.
	 *
	 * @var array $description Reusable description.
	 */
	 ...
	 *
	 * @var array $props Property array.
	 */
	...
	/**
	 * Inviolate Properties
	 *
	 * List any keys from $props that should not overwritable.
	 *
	 * @var array $immutable Immutable, unchanging properties.
	 */
	 ...
	 *    }
	 *
	 * Properties are passed to tpls and [+placeholders+] are replaced.
	 *
	 *    $config = Array (
	 *        'id_prefix' =>
	 *        'name_prefix' =>
	...
	public function __construct() {
		...
		$this->descriptions['button_label']           = __(
			'How should the button be labeled? (Users will click this button to select the image, media, or relation).',
			CCTM_TXTDOMAIN
		);
		$this->descriptions['class']                  = __(
			'Add a CSS class to instances of this field. Use this to customize styling in the WP manager.',
			CCTM_TXTDOMAIN
		);
		$this->descriptions['extra']                  = __(
			'Any extra attributes for this text field, e.g. <code>size="10"</code>',
			CCTM_TXTDOMAIN
		);
		$this->descriptions['default_option']         = __(
			'The default option will appear selected. Make sure it matches a defined option.',
			CCTM_TXTDOMAIN
		);
		$this->descriptions['default_value']          = __(
			'The default value is presented to users when a new post is created.',
			CCTM_TXTDOMAIN
		);
		$this->descriptions['description']            = __(
			'The description is visible when you view all custom fields or when you use the <code>get_custom_field_meta()</code> function.'
		);
		$this->descriptions['description']           .= __( 'The following html tags are allowed:' )
			. '<code>'
			. htmlspecialchars( CCTM::$allowed_html_tags )
			. '</code>';
		$this->descriptions['evaluate_default_value'] = __(
			'You can check this box if you want to enter a bit of PHP code into the default value field.'
		);
		$this->descriptions['label']                  = __(
			'The label is displayed when users create or edit posts that use this custom field.',
			CCTM_TXTDOMAIN
		);
		$this->descriptions['name']                   = __(
			'The name identifies the meta_key in the wp_postmeta database table. The name should contain only letters, numbers, and underscores. You will use this name in your template functions to identify this custom field.',
			CCTM_TXTDOMAIN
		);
		$this->descriptions['name']                  .= sprintf(
			'<br /><span style="color:red;">%s</span>',
			__(
				'WARNING: if you change the field name, you will have to update any functions that reference this field by name, e.g. <code>get_custom_field()</code>, <code>print_custom_field()</code>,  or any search criteria.',
				CCTM_TXTDOMAIN
			)
		);
		$this->descriptions['is_repeatable']          = __(
			'If selected, the user will be able to enter multiple instances of this field, e.g. multiple images. Your templates will need to handle formatting an array of values, e.g. via the "to_array" or other output filters, even if you only use one instance of the field.',
			CCTM_TXTDOMAIN
		);
		$this->descriptions['required']               = __(
			'If checked, users must add a value to this field before the page can be published.',
			CCTM_TXTDOMAIN
		);
		$this->descriptions['checked_value']          = __(
			'What value should be stored in the database when this checkbox is checked?',
			CCTM_TXTDOMAIN
		);
		$this->descriptions['unchecked_value']        = __(
			'What value should be stored in the database when this checkbox is unchecked?',
			CCTM_TXTDOMAIN
		);
		$this->descriptions['checked_by_default']     = __(
			'Should this field be checked by default?',
			CCTM_TXTDOMAIN
		);
		$this->descriptions['output_filter']          = __(
			'How should values be displayed in your theme files?',
			CCTM_TXTDOMAIN
		);
		$this->descriptions['use_key_values']         = __(
			'Check this to make the stored values distinct from the options displayed to the user, e.g. Option:"Red", Stored Value:"#ff0000;"',
			CCTM_TXTDOMAIN
		);
	}
	 ...
	 *
	 * @param string $k Controls properties, part 1.
	 * @return string
	 */
	 ...
	 * This is a magic interface to "controlled" class properties in $this->props
	 *
	 * @param string $k Controls properties, part 2.
	 * @return boolean
	 */
	...
	public function __set( $k, $v ) {
		if ( ! in_array( $k, $this->immutable, true ) ) {
			$this->props[ $k ] = $v;
		}
	}
	...
	// -----------------------------------------------------------------------------!
	/**
	 * _get_search_parameters_visible
	 *
	...
	public static function valid_name( $name ) {
		...
		if ( ! preg_match( '/^[a-z]{1}[a-z_0-9]*$/i', $name ) ) {
			self::$e[] = sprintf(
				__(
					'%s contains invalid characters. The name may only contain letters, numbers, and underscores, and it must begin with a letter.',
					CCTM_TXTDOMAIN
				),
				'<strong>' . $name . '</strong>'
			);
		...
		if ( strlen( $name ) > 255 ) {
			self::$e[] = __(
				'The name is too long. Names must not exceed 255 characters.',
				CCTM_TXTDOMAIN
			);
		}
		// Run into any reserved words?
		if ( in_array( $name, CCTM::$reserved_field_names, true ) ) {
			self::$e[] = sprintf(
				__(
	...
	// ------------------------------------------------------------------------------!
	/**
	 * Format Available Output Filters
	 *
			...
			<div class="inside">
				<div class="' . self::WRAPPER_CSS_CLASS . '" id="output_filter_wrapper">
				 	<label for="output_filter" class="cctm_label cctm_select_label" id="output_filter_label">'
						...
						. '<a href="http://code.google.com/p/wordpress-custom-content-type-manager/wiki/OutputFilters" target="_blank">
							<img src="' . CCTM_URL . '/images/question-mark.gif" width="16" height="16" />
						</a>
				 	</label>';

			$out .= '
					<select name="output_filter" class="cctm_select" id="output_filter">
	 ...
	 * the ones that represent the attributes that every field will have, e.g.
	 * Label, Name, Default Value, Extra, Class, is Repeatable, Description.
	 *
	 * @param array $def Current def.
	 * @param bool  $show_repeatable Shows repeating items.
	 * @return string HTML.
	 */
	public function format_standard_fields( $def, $show_repeatable = true ) {
		$is_checked = '';
		if ( isset( $def['is_repeatable'] ) && 1 === $def['is_repeatable'] ) {
		...
		// Label.
		$out .= '
					<div class="' . self::WRAPPER_CSS_CLASS . '" id="label_wrapper">
			 			<label for="label" class="' . self::LABEL_CSS_CLASS . '">'
							. __( 'Label', CCTM_TXTDOMAIN )
						. '</label>
		...
		// Name.
		$out .= '
					<div class="' . self::WRAPPER_CSS_CLASS . '" id="name_wrapper">
		...
		$out .= '
					<div class="' . self::WRAPPER_CSS_CLASS . '" id="default_value_wrapper">
			 			<label for="default_value" class="cctm_label cctm_text_label" id="default_value_label">'
							. __( 'Default Value', CCTM_TXTDOMAIN )
						. '</label>
		...
		// Extra.
		$out .= '
					<div class="' . self::WRAPPER_CSS_CLASS . '" id="extra_wrapper">
			 			<label for="extra" class="' . self::LABEL_CSS_CLASS . '">'
							. __( 'Extra', CCTM_TXTDOMAIN )
						. '</label>
						<input type="text" name="extra" class="cctm_text" id="extra" value="' . htmlspecialchars( $def['extra'] ) . '" />'
						. $this->get_translation( 'extra' )
					. '</div>';

		// Class.
		$out .= '
					<div class="' . self::WRAPPER_CSS_CLASS . '" id="class_wrapper">
			 			<label for="class" class="' . self::LABEL_CSS_CLASS . '">'
							. __( 'Class', CCTM_TXTDOMAIN )
						. '</label>
						<input type="text" name="class" class="cctm_text" id="class" value="' . htmlspecialchars( $def['class'] ) . '" />'
		...
		if ( $show_repeatable ) {
			// Is Repeatable?
			$out .= '
						<div class="' . self::WRAPPER_CSS_CLASS . '" id="is_repeatable_wrapper">
							<label for="is_repeatable" class="cctm_label cctm_checkbox_label" id="is_repeatable_label">'
								. __( 'Is Repeatable?', CCTM_TXTDOMAIN )
							. '</label>
							<br />
							<input type="checkbox" name="is_repeatable" class="cctm_checkbox" id="is_repeatable" value="1" ' . $is_checked . ' />
							<span>' . $this->descriptions['is_repeatable'] . '</span>
				 		</div>';
		}

		// Description.
		$out .= '
						<div class="' . self::WRAPPER_CSS_CLASS . '" id="description_wrapper">
			 				<label for="description" class="' . self::LABEL_CSS_CLASS . '">'
								. __( 'Description', CCTM_TXTDOMAIN )
							. '</label>
	 ...
	 *
	 * @param array $def Current def.
	 * @param bool  $show_validators Show validation.
	 * @return string HTML.
	 */
					...
					<tr>
						<td style="vertical-align:top">
							...
							<div class="' . self::WRAPPER_CSS_CLASS . '" id="required_wrapper">
								...
								'</label>
								<br />
								<input type="checkbox" name="required" class="cctm_checkbox" id="required" value="1" ' . $req_is_checked . ' />
								<span class="cctm_checkbox_label">' . $this->descriptions['required'] . '</span>
							</div>'
							. $validation_select . '
						</td>
						<td width="100"></td>
	...
	// -----------------------------------------------------------------------------!
	/**
	 * Create/Edit/Get Field Definition
	 *
	...
	// ------------------------------------------------------------------------------!
	/**
	 * Edit or Get Field Instance
	 *
	 * The form returned is what is displayed when a user is editing a post that contains
	 * an instance of this field type.
	 *
	 * @param string $current_value is the current value for the field, as stored in the wp_postmeta table for the post being edited.
	...
	// -----------------------------------------------------------------------------!
	/**
	 * Edit or Get a Field Definition
	 *
	 ...
	 *
	 * Note that the field name in the $_POST array is prefixed with self::POST_NAME_PREFIX,
	 * e.g. the value for you 'my_field' custom field is stored in $_POST['cctm_my_field']
	 * (where self::POST_NAME_PREFIX = 'cctm_'). This is done to avoid name
	...
	public function save_definition_filter( $posted_data ) {
		...
		if ( empty( $posted_data['name'] ) ) {
			...
			// Run into any reserved words?
			if ( in_array( $posted_data['name'], CCTM::$reserved_field_names, true ) ) {
				$this->errors['name'][] = sprintf(
					__(
						'%s is a reserved name.',
						CCTM_TXTDOMAIN
				...
				if ( isset( CCTM::$data['custom_field_defs'] ) && is_array( CCTM::$data['custom_field_defs'] ) ) {
					foreach ( CCTM::$data['custom_field_defs'] as $cf => $def ) {
						if ( strtolower( $posted_data['name'] ) === strtolower( $cf ) ) {
							$this->errors['name'][] = sprintf(
								__(
									'The name %s is already in use. Please choose another name.',
									CCTM_TXTDOMAIN
								),
								'<em>' . $posted_data['name'] . '</em>'
							);
						}
					}
				}
			} elseif ( $this->original_name !== $posted_data['name'] ) { // It iss an EDIT operation and we're renaming the field.
				if ( isset( CCTM::$data['custom_field_defs'] ) && is_array( CCTM::$data['custom_field_defs'] ) ) {
					if ( strtolower( $posted_data['name'] ) === strtolower( $cf ) ) {
						$this->errors['name'][] = sprintf(
							__(
	...
	public function set_prop( $key, $value ) {
		if ( is_scalar( $key ) ) {
			...
			$this->errors['improper_input_set_props'] = __(
				'Improper input to the set_prop() function.',
				CCTM_TXTDOMAIN
			);
			return false;
		}
	 ...
	 * Shepherded access to the $this->props array.
	 *
	 * @param array $array Set properties.
	 */
	public function set_props( $array ) {
		if ( ! is_array( $array ) ) {
			$this->errors['improper_input_set_props'] = __(
				'Improper input to the set_props() function.',
				CCTM_TXTDOMAIN
			);
			return false;
		}
