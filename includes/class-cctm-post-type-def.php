<?php
/**
 * CCTM Post Type Definition file
 *
 * PHP 7.2+
 *
 * @category Component
 * @package CCTM
 * @subpackage CCTM_Post_Type_Def
...
class CCTM_Post_Type_Def {
	...
	public static function get_columns( $post_type ) {
		...
		foreach ( $custom_columns as $c ) {
			...
			} elseif ( in_array( $c, $custom_fields, true ) ) {
		...
		foreach ( $built_in_columns as $c => $label ) {
			if ( in_array( $c, $custom_columns, true ) ) {
				continue;
			}
		...
		foreach ( $taxonomies as $taxonomy ) {
			if ( in_array( $taxonomy, $custom_columns, true ) ) {
				continue;
			}
	 ...
	 *
	 * @param string $post_type Post name.
	 */
	public static function get_orderby_options( $post_type ) {
		...
		foreach ( $built_in_columns as $c ) {
			$is_selected = '';
			if (
				isset( CCTM::$data['post_type_defs'][ $post_type ]['custom_orderby'] )
	 ...
	 * Get Post Type Icons
	 *
	 * Representing all img tags for all post-type icons.
	 *
	 * @return string
	 */
	public static function get_post_type_icons() {
		...
		$icons  = array();
		$handle = opendir( CCTM_PATH . '/images/icons/16x16' );
	 ...
	 * @param bool   $search_built_ins (optional) Whether or not to search ANY defined post-type.
	...
	public static function is_existing_post_type( $post_type, $search_built_ins = true ) {
		...
		if ( empty( CCTM::$data['post_type_defs'] ) && $search_built_ins ) {
			return in_array( $post_type, CCTM::$built_in_post_types, true );
		} elseif ( empty( CCTM::$data['post_type_defs'] ) && ! $search_built_ins ) {
			// If there is no existing $data and we omit the built-ins...
			return false;
		} elseif ( array_key_exists( $post_type, CCTM::$data['post_type_defs'] ) ) {
			// Check to see if we've stored this $post_type before.
			return true;
		} elseif ( $search_built_ins && in_array( $post_type, get_post_types(), true ) ) {
			// Check the built-ins.
			return true;
		} else {
			return false;
	...
	public static function post_type_name_has_errors( $data, $new = false ) {
		...
		// Remember: the location in the $_POST array is different from the name of the option in the form-def.
		if ( empty( $data['labels']['menu_name'] ) ) {
			return __( 'Menu Name is required.', CCTM_TXTDOMAIN );
		}

		foreach ( CCTM::$reserved_prefixes as $rp ) {
			if ( preg_match( '/^' . preg_quote( $rp, $delimiter ) . '.*/', $data['post_type'] ) ) {
				return sprintf(
					__(
		...
		foreach ( $cctm_post_types as $pt ) {
			if (
				isset( CCTM::$data['post_type_defs'][ $pt ]['is_foreign'] )
				&& CCTM::$data['post_type_defs'][ $pt ]['is_foreign']
				&& ! in_array( $pt, $registered_post_types, true )
			) {
				$dead_foreigners[] = $pt;
			}
		}
		...
		// Is reserved name?
		if ( in_array( $data['post_type'], CCTM::$reserved_post_types, true ) ) {
			$msg  = __(
		...
		elseif ( in_array( $data['post_type'], $taxonomy_names_array, true ) ) {
			$msg  = __(
				'Please choose another name.',
				CCTM_TXTDOMAIN
			);
			...
			$msg .= sprintf(
				__(
					'%s is already in use as a registered taxonomy name.',
					CCTM_TXTDOMAIN
				),
				$post_type
			);
			 ...
			 */
			$new
			&& is_array( CCTM::$data['post_type_defs'] )
			&& in_array( $data['post_type'], $cctm_post_types, true )
			&& ! in_array( $data['post_type'], $dead_foreigners, true )
		) {
			return sprintf(
				__(
		...
		} elseif ( in_array( $data['post_type'], $other_post_types, true ) ) {
			// Is the name taken by an existing post type registered by some other plugin?
			return sprintf(
				__(
					'The name %s has been registered by some other plugin.',
					CCTM_TXTDOMAIN
				),
				htmlspecialchars( $data['post_type'] )
			);
		 ...
		 *    return sprintf(
		 *       __(
		 *           'There is a template file named single-%s.php in your theme directory (%s).',
		 *           CCTM_TXTDOMAIN
		 *      ),
		 *      htmlspecialchars( $data['post_type'] ),
	...
	public static function sanitize_post_type_def( $raw ) {
		...
		if ( empty( $sanitized['labels']['not_found'] ) ) {
			$sanitized['labels']['not_found'] = sprintf(
				__(
					'No %s found',
					CCTM_TXTDOMAIN
				),
				strtolower( $raw['labels']['menu_name'] )
			);
		}

		if ( empty( $sanitized['labels']['not_found_in_trash'] ) ) {
			$sanitized['labels']['not_found_in_trash'] = sprintf(
				__(
					'No %s found in trash',
					CCTM_TXTDOMAIN
				),
	 ...
	 *
	 * This saves a serialized data structure (arrays of arrays) to the database.
	 *
